...
class WatchFaceBackground extends WatchUi.Drawable {
  ...
  private function drawHourHand(dc as Graphics.Dc, clockTime as System.ClockTime) {
    var hour = clockTime.hour;
    var minutes = clockTime.min;
    var hourAngle = ((hour % 12).toFloat() + minutes.toFloat() / 60.0) * 30.0; // 360 / 12 = 30

    // hour hand
    var hourHandPoints = WatchFaceHelpers.getHandPoints(dc, _radius, 0.1, hourAngle, [
      [0.044, -0.05],
      [0.067, -0.6],
      [0.024, -0.7],
      [0.0, -0.84],
    ...
    var hourHandDecorationPoints = WatchFaceHelpers.getHandPoints(dc, _radius, -0.48, hourAngle, [
      [0.028, 0.0],
      [0.0, -0.076],
      [-0.028, 0.0],
    ]);
  ...
  private function drawMinuteHand(dc as Graphics.Dc, clockTime as System.ClockTime) {
    var minutes = clockTime.min;
    var seconds = clockTime.sec;
    var minuteAngle = (minutes.toFloat() + seconds.toFloat() / 60.0) * 6.0; // 360 / 60 = 6


        var minuteHandPoints = WatchFaceHelpers.getHandPoints(dc, _radius, 0.2, minuteAngle, [
      [0.08, -0.06],
      [0.038, -0.8],
      [0.12, -0.8],
      [0.028, -0.96],
    ...
    var minuteHandDecorationPoints = WatchFaceHelpers.getHandPoints(dc, _radius, -0.64, minuteAngle, [
      [0.052, 0.0],
      [0.0, -0.08],
      [-0.052, 0.0],
    ]);
  ...
  private function drawSecondHand(dc as Graphics.Dc, clockTime as System.ClockTime) {
    var seconds = clockTime.sec;
    var secondAngle = seconds.toFloat() * 6; // 360 / 60 = 6

    var offsetY = 0.34;
    var secondHandPoints = WatchFaceHelpers.getHandPoints(dc, _radius, offsetY, secondAngle, [
      [0.02, 0.0],
      [0.0, -1.14],
      [-0.02, 0.0],
    ]);
    ...
    dc.drawCircle(counterWeightCirclePoint[0], counterWeightCirclePoint[1], 0.058 * _radius); // counterweight circle
    dc.drawCircle(_radius, _radius, 0.058 * _radius); // center circle

    // fill
    dc.setColor(_handColor, Graphics.COLOR_TRANSPARENT);
    dc.fillPolygon(secondHandPoints); // hand
    dc.fillCircle(counterWeightCirclePoint[0], counterWeightCirclePoint[1], 0.058 * _radius); // counterweight circle
    ...
    dc.setColor(_logoColor, Graphics.COLOR_TRANSPARENT);
    dc.fillCircle(counterWeightCirclePoint[0], counterWeightCirclePoint[1], 0.028 * _radius); // counterweight circle
  }
